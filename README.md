# Patch Notes

A satirical card game where you ship updates players and investors can disagree on in the language of professionally deniable hostility.

You play as the person deciding what goes into each update. Every card is a proposed change to the game: some make players happier, some make investors happier, and some make the word "better" depend heavily on who is reading the report.

Each release, choose which updates to ship and which to discard. After 10 releases, the game looks back at what your decisions did to Fun and Money, then gives you the kind of ending a product team would describe as "a learning opportunity."

## Play

- Latest development build: [GitHub Pages](https://josephblt.github.io/patch-notes/)
- Official jam version: [itch.io](https://josephblt-games.itch.io/patch-notes)

## Design

The current game design document lives in [GAME_DESIGN_DOCUMENT.md](GAME_DESIGN_DOCUMENT.md).

## Jam

Patch Notes is being made for [Pirate Software - Game Jam 19](https://itch.io/jam/pirate).

Theme: **The Cost of Progress**

The jam theme asks what has to be paid as progress continues. Patch Notes interprets that through game updates: every improvement has a cost, and the bill lands somewhere between the players, the investors, and the poor soul writing the release notes.
