...
@onready var _git_ref_link: LinkButton = $VersionLeftVBoxContainer/VersionLine6
@onready var _commit_link: LinkButton = $VersionLeftVBoxContainer/VersionLine7
@onready var _run_link: LinkButton = $VersionLeftVBoxContainer/VersionLine8
...
func _ready() -> void:
	...
	_set_metadata_link(_git_ref_link, "    Git Ref: %s" % metadata.git_ref, metadata.get_git_ref_url())
	_set_metadata_link(_commit_link, "    Commit: %s" % metadata.commit_sha, metadata.get_commit_url())
	_set_metadata_link(_run_link, "    Run: %s" % metadata.get_run_display(), metadata.get_run_url())
